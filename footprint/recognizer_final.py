...
def count(thresholded, segmented,loaded_model):
...
def mainwork():
    ...
    # 從鏡頭拍攝圖像 ret:代表是否成功 frame :攝影機單張畫面
        ...
        if num_frames < 30:
            ...
            hand = segment(gray)
                # 判斷是否有手，若無則結束程式
            if num_frames > 85 and hand is None:
                time.sleep(5)
                camera.release()
                cv2.destroyAllWindows()
                import Speech_To_Text
                return Speech_To_Text.speech()
                break
            ...
            if hand is not None:
                ...
                fingers = count(thresholded, segmented,loaded_model)
